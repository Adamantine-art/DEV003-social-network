...
class Q {
  ...
  getToken() {
    ...
    return this.forceRefresh = false, this.auth ? this.auth.getToken(e).then((e2) => (
      ...
      this.i !== t2 ? (C("FirebaseAuthCredentialsProvider", "getToken aborted due to token change."), this.getToken()) : e2 ? (M("string" == typeof e2.accessToken), new U(e2.accessToken, this.currentUser)) : null
    ...
    return function(t4, e2, n2, s2) {
      ...
      return r.forEach((t5) => {
        o = o.next(() => s2.getEntry(e2, t5)).next((e3) => {
          ...
          M(null !== r2), e3.version.compareTo(r2) < 0 && (i2.applyToRemoteDocument(e3, n2), e3.isValidDocument() && // We use the commitVersion as the readTime rather than the
...
function jo(t2, e, n) {
  ...
  return n.forEach((t3) => s = s.add(t3)), e.getEntries(t2, s).next((t3) => {
    ...
    return n.forEach((n2, r) => {
      ...
      ) : !o.isValidDocument() || r.version.compareTo(o.version) > 0 || 0 === r.version.compareTo(o.version) && o.hasPendingWrites ? (e.addEntry(r), s2 = s2.insert(n2, r)) : C("LocalStore", "Ignoring outdated watch update for ", n2, ". Current version:", o.version, " Watch version:", r.version);
    }), {
      Wi: s2,
      zi: i
    };
...
class {
   ...
   */
  lo(t2, e, n) {
    t2["X-Goog-Api-Client"] = "gl-js/ fire/" + v, // Content-Type: text/plain will avoid preflight requests which might
    ...
    return f(c, WebChannel.EventType.OPEN, () => {
      ...
      if (!h) {
        ...
        if (i2) {
          ...
          void 0 === e3 && (e3 = B.INTERNAL, n3 = "Unknown error status: " + t4 + " with message " + i2.message), // Mark closed so no further events are propagated
          h = true, l2.io(new L(e3, n3)), c.close();
        } else
...
class Iu {
  ...
  Ro(t2) {
    ...
    s > 0 && C("ExponentialBackoff", `Backing off for ${s} ms (base delay: ${this.Io} ms, delay with jitter: ${e} ms, last attempt: ${n} ms ago)`), this.To = this.Hs.enqueueAfterDelay(this.timerId, s, () => (this.Eo = Date.now(), t2())), // Apply backoff factor to determine next delay and ensure it is within
    // bounds.
...
class Tu {
  ...
  async close(t2, e) {
    ...
    this.So++, 4 !== t2 ? (
      ...
      (x(e.toString()), x("Using maximum backoff delay to prevent overloading the backend."), this.xo.Ao())
    ) : e && e.code === B.UNAUTHENTICATED && 3 !== this.state && // "unauthenticated" error means the token was rejected. This should rarely
...
class Au extends Tu {
  ...
  onMessage(t2) {
    if (
      ...
      M(!!t2.streamToken), this.lastStreamToken = t2.streamToken, this.Jo
    ) {
      this.xo.reset();
...
async function qu(t2, e, n) {
  ...
  t2.asyncQueue.enqueueRetryable(async () => {
    C("RemoteStore", "Retrying IndexedDB access"), await n(), t2._u.delete(
      1
      /* OfflineCause.IndexedDbFailed */
    ), await Vu(t2);
  });
...
async function Yu(t2, e) {
  e && ec(t2).Yo && // This error affects the actual write.
  await async function(t3, e2) {
    if (n = e2.code, as(n) && n !== B.ABORTED) {
      const n2 = t3.fu.shift();
      ec(t3).Mo(), await Uu(t3, () => t3.remoteSyncer.rejectFailedWrite(n2.batchId, e2)), // It's possible that with the completion of this mutation
...
function ec(t2) {
  ...
  (t2.Iu = function(t3, e, n) {
    ...
    await Ku(t2)) : (await t2.Iu.stop(), t2.fu.length > 0 && (C("RemoteStore", `Stopping write stream with ${t2.fu.length} pending writes`), t2.fu = []));
  })), t2.Iu;
}
/**
 * @license
...
class nc {
  constructor(t2, e, n, s, i) {
    this.asyncQueue = t2, this.timerId = e, this.targetTimeMs = n, this.op = s, this.removalCallback = i, this.deferred = new q(), this.then = this.deferred.promise.then.bind(this.deferred.promise), // It's normal for the deferred promise to be canceled (due to cancellation)
...
class Tc {
  constructor(t2, e) {
    ...
    this.mutatedKeys = Es(), this.Gu = bn(t2), this.Qu = new ic(this.Gu);
  }
  /**
...
async function Sc(t2, e) {
  ...
  try {
    ...
    e.targetChanges.forEach((t4, e2) => {
      ...
      (M(t4.addedDocuments.size + t4.modifiedDocuments.size + t4.removedDocuments.size <= 1), t4.addedDocuments.size > 0 ? s.nc = true : t4.modifiedDocuments.size > 0 ? M(s.nc) : t4.removedDocuments.size > 0 && (M(s.nc), s.nc = false));
    }), await Uc(n, t3, e);
  } catch (t3) {
    await Et(t3);
  }
...
async function Cc(t2, e, n) {
  ...
  if (r) {
    ...
    await Sc(s, i2), // Since this query failed, we won't want to manually unlisten to it.
...
async function xc(t2, e) {
  ...
  try {
    ...
    Mc(
      ...
      /*error=*/
      null
    ), Oc(n, s), n.sharedClientState.updateMutationState(s, "acknowledged"), await Uc(n, t3);
  } catch (t3) {
    await Et(t3);
  }
}
...
async function Kc(t2, e) {
  ...
  if (!n.currentUser.isEqual(e)) {
    ...
    n.sharedClientState.handleUserChange(e, t3.removedBatchIds, t3.addedBatchIds), await Uc(n, t3.ji);
  }
}
function Gc(t2, e) {
...
class oa {
  async initialize(t2, e) {
    this.localStore || (this.localStore = t2.localStore, this.sharedClientState = t2.sharedClientState, this.datastore = this.createDatastore(e), this.remoteStore = this.createRemoteStore(e), this.eventManager = this.createEventManager(e), this.syncEngine = this.createSyncEngine(
      ...
      /* startAsPrimary=*/
      !t2.synchronizeTabs
    ), this.sharedClientState.onlineStateHandler = (t3) => Dc(
      this.syncEngine,
      t3,
      1
      /* OnlineStateSource.SharedClientState */
    ), this.remoteStore.remoteSyncer.handleCredentialChange = Kc.bind(null, this.syncEngine), await Zu(this.remoteStore, this.syncEngine.isPrimaryClient));
  }
  ...
  createRemoteStore(t2) {
    return e = this.localStore, n = this.datastore, s = t2.asyncQueue, i = (t3) => Dc(
      this.syncEngine,
      t3,
      0
  ...
  terminate() {
    return async function(t2) {
      const e = $(t2);
      C("RemoteStore", "RemoteStore shutting down."), e._u.add(
        5
...
class ma {
  ...
  constructor(t2, e, n, s) {
     ...
     */
    this.type = "firestore-lite", this._persistenceKey = "(lite)", this._settings = new wa({}), this._settingsFrozen = false;
  }
  /**
...
function Aa(t2, e, ...n) {
  ...
  // 'undefined' and 'null'.
  1 === arguments.length && (e = X.R()), ua("doc", "path", e), t2 instanceof ma) {
    const s = rt.fromString(e, ...n);
    return aa(s), new ya(
      t2,
      /* converter= */
...
const onNavigate = (pathname) => {
  ...
  root2.appendChild(routes[pathname]());
};
const reviews = () => {
  ...
  albumTitle.textContent = "Nombre de \xC1lbum";
  ...
  backArrow.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/arrow-left.png";
  ...
  editIcon.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/edit-pencil.png";
  const deleteIcon = document.createElement("img");
  deleteIcon.className = "delete-icon";
  deleteIcon.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/trash.png";
  ...
  postReviewButton.addEventListener("click", () => {
    ...
    createReview(user, commentField).then((result) => {
      alert("exito", result);
      showReviews();
    }).catch((err) => {
      if (commentField === "") {
        alert("Campo vac\xEDo", err);
      }
    });
  });
...
const db = oh(app);
function createReview(username, comment) {
  const dbRef = Aa(Ta(db, "reviews"));
  const data = {
    username,
    comment,
    likes: []
  };
  const result = Ul(dbRef, data);
  return result;
}
const getReview = () => Bl(Ta(db, "reviews"));
const deleteReview = () => Gl(Aa(db, "reviews", "documentId"));
const signUp = (props = {}) => {
  const desktopContainer = document.createElement("div");
  desktopContainer.className = "desktop-container";
  const desktopImg = document.createElement("img");
  desktopImg.className = "desktop-img";
  desktopImg.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/dj.png?raw=true";
  desktopImg.id = "desktopImg";
  const divSignUp = document.createElement("div");
  divSignUp.className = "sign-up-container";
  const logo = document.createElement("img");
  logo.className = "logo";
  logo.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/logo%20mapa%20de%20bits.png";
  ...
  email.textContent = "Correo electr\xF3nico";
  ...
  password.textContent = "Constrase\xF1a";
  ...
  buttonLogIn.textContent = "Inicia Sesi\xF3n con Google";
  ...
  bottomText.textContent = "\xBFYa tienes una cuenta?";
  const logInText = document.createElement("button");
  logInText.textContent = "Inicia sesi\xF3n ac\xE1";
  logInText.className = "log-in-text";
  logInText.id = "logInText";
  buttonSignUp.addEventListener("click", () => {
    const userEmail = document.getElementById("emailInput").value;
    const userPassword = document.getElementById("passwordInput").value;
    signUpFirebase(userEmail, userPassword).then((userCredential) => {
      ...
      if (errorCode === "auth/weak-password") {
        errorContent.innerHTML = "La contrase\xF1a debe tener al menos 6 car\xE1cteres";
...
const logIn = () => {
  ...
  const arrow = document.createElement("img");
  arrow.className = "arrow";
  arrow.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/flechita.png";
  const bottomTextContainer = document.createElement("div");
  bottomTextContainer.className = "bottom-text-container";
  const bottomText = document.createElement("p");
  bottomText.className = "bottom-text";
  bottomText.textContent = "\xBFNo tienes una cuenta?";
  const logInText = document.createElement("p");
  logInText.textContent = "Reg\xEDstrate ac\xE1";
  logInText.className = "log-in-text";
  logInText.id = "logInText";
  buttonLogin.addEventListener("click", () => {
    const userEmail = document.getElementById("emailInput").value;
...
const home = () => {
  ...
  logoHome.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/logo%20mapa%20de%20bits.png";
  const userIcon = document.createElement("img");
  userIcon.className = "user";
  userIcon.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/user.png";
  ...
  album2.textContent = "\xC1lbum";
  album2.className = "albums";
  const artists = document.createElement("div");
  artists.textContent = "Artistas";
  artists.className = "artists";
  const genres = document.createElement("div");
  genres.textContent = "G\xE9neros";
  ...
  categoriesColumn1.append(favorites, trending, album2);
...
const album = () => {
  ...
  titleMenuAlbum.textContent = "Lista de \xE1lbumes";
  ...
  titleOptionAlbum.textContent = "\xC1lbumes";
  ...
  imgChatBuble.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/chat-bubble.png";
  imgChatBuble.type = "button";
  const imgHeart = document.createElement("img");
  imgHeart.className = "img-heart";
  imgHeart.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/Vector.png";
  const moreOptions = document.createElement("img");
  moreOptions.className = "more-options";
  moreOptions.src = "https://github.com/fabihasu/DEV003-social-network/blob/main/src/img/more-vert.png";
